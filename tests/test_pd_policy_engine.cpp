...
void iterateThoughExpectedStates(std::vector<int> expectedStates) {
  ...
  CHECK_EQUAL(expectedStates.size(), iterationCounter + 1);
  CHECK_EQUAL(expectedStates[iterationCounter], pe.currentStateCode());
}
...
void test_NormalNegotiation() {
  ...
  iterateThoughExpectedStates({6, 7, 8, 0, 0});
  ...
  pe.IRQOccured();
  iterateThoughExpectedStates({1, 0, 0});
  ...
  iterateThoughExpectedStates({2, 9, 0, 0});
  ...
  iterateThoughExpectedStates({10, 0, 0});
  // Now send the PS_RDY
  injectTestmessage(sizeof(message_ready), message_ready);
  iterateThoughExpectedStates({11, 12, 0, 0});
}
...
TEST(PD, PDNegotiationTest) {
  ...
  iterateThoughExpectedStates({4, 5, 0, 0});
  ...
  iterateThoughExpectedStates({12, 14, 0, 0});
