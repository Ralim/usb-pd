...
uint8_t
    PolicyEngine::ucQueueStorageArea[PDB_MSG_POOL_SIZE * sizeof(union pd_msg)];
...
PolicyEngine::policy_engine_state PolicyEngine::pe_sink_wait_cap() {
  ...
  EventBits_t evt =
      waitForEvent((uint32_t)Notifications::PDB_EVT_PE_MSG_RX |
                       (uint32_t)Notifications::PDB_EVT_PE_I_OVRTEMP |
                       (uint32_t)Notifications::PDB_EVT_PE_RESET,
...
PolicyEngine::policy_engine_state PolicyEngine::pe_sink_select_cap() {
  ...
  EventBits_t evt = waitForEvent((uint32_t)Notifications::PDB_EVT_PE_TX_DONE |
                                 (uint32_t)Notifications::PDB_EVT_PE_TX_ERR |
                                 (uint32_t)Notifications::PDB_EVT_PE_RESET);
  /* If we got reset signaling, transition to default */
  ...
  if ((evt & (uint32_t)Notifications::PDB_EVT_PE_TX_ERR) ==
      (uint32_t)Notifications::PDB_EVT_PE_TX_ERR) {
    return PESinkHardReset;
  }

  /* Wait for a response */
  evt = waitForEvent((uint32_t)Notifications::PDB_EVT_PE_MSG_RX |
                         (uint32_t)Notifications::PDB_EVT_PE_RESET,
...
PolicyEngine::policy_engine_state PolicyEngine::pe_sink_ready() {
  ...
  if (evt & (uint32_t)Notifications::PDB_EVT_PE_MSG_RX) {
    if (messageWaiting()) {
      ...
      if (PD_MSGTYPE_GET(&tempMessage) == PD_MSGTYPE_VENDOR_DEFINED &&
      ...
      } else if (PD_MSGTYPE_GET(&tempMessage) == PD_MSGTYPE_SINK_CAPABILITIES &&
                 PD_NUMOBJ_GET(&tempMessage) > 0) {
        return PESinkSendNotSupported;
        ...
        if ((tempMessage.hdr & PD_HDR_EXT) &&
            (PD_DATA_SIZE_GET(&tempMessage) > PD_MAX_EXT_MSG_LEGACY_LEN)) {

          return PESinkChunkReceived;
          /* Tell the DPM a message we sent got a response of
           * Not_Supported. */
        } else if (PD_MSGTYPE_GET(&tempMessage) == PD_MSGTYPE_NOT_SUPPORTED &&
                   PD_NUMOBJ_GET(&tempMessage) == 0) {
...
PolicyEngine::policy_engine_state PolicyEngine::pe_sink_chunk_received() {
  ...
  EventBits_t evt = waitForEvent((uint32_t)Notifications::PDB_EVT_PE_RESET,
...
PolicyEngine::policy_engine_state
PolicyEngine::pe_sink_not_supported_received() {
...
EventBits_t PolicyEngine::waitForEvent(uint32_t mask, TickType_t ticksToWait) {
